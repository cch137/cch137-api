...
import htmlTableTo2DArray from "../../utils/htmlTableTo2DArray.js";

const url =
  "https://docs.google.com/spreadsheets/d/1VjdmK8-PBoOt6sRCvZ40G5qmYyxpLzHl5RCSQWejxYk/preview/sheet?gid=1360697588";
let lastFetched = 0;
...
async function _fetchCurrencies() {
  if (fetching === null) {
    fetching = (async () => {
      ...
      const table = htmlTableTo2DArray(res.data).map((r) =>
        r.slice(1, 4)
      ) as string[][];
      currencyMap.clear();
      currenctSet.clear();
