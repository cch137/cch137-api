...
function htmlTableTo2DArray(html: string, options: ParseTableOptions = {}): ParsedTable {
  const {
    removeEmptyRow = true,
    removeRepeatedNullCharacters = true
  } = options
  const $ = load(html);
      ...
      .each(function (cellIndex, _cellElement) {
        ...
        if (tableInCell.length !== 0) {
          row.push([...tableInCell].map(_table => {
            ...
            if (!_tableHtml.includes('<table>')) _tableHtml = `<table>${$(_table).html()}</table>`;
