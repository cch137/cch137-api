...
type McStat = {
  ...
  players: { max: number; online: number; sample: [] };
  description: string;
  favicon: string;
};
...
export default class MinecraftServerListPing {
  ...
  static ping(protocol = 4, host: string, port = 25565, timeout = 3000) {
    return new Promise<McStat>((resolve, reject) => {
  ...
  static ping16(protocol = 73, host: string, port = 25565, timeout = 3000) {
    return new Promise((resolve, reject) => {
      ...
      client.on("timeout", (/* error */) => {
        ...
        reject(
          new Error(
            "The client timed out while connecting to " +
              host +
              ":" +
              port +
              "."
      ...
      client.on("connect", () => {
        ...
        let requestBuffer = Buffer.from([
          ...
          0x00, 0x50, 0x00, 0x69, 0x00, 0x6e, 0x00, 0x67, 0x00, 0x48, 0x00,
          0x6f, 0x00, 0x73, 0x00, 0x74,
        ]);
      ...
      // Response
      client.on("data", (responseBuffer) => {
        const dataBuffer = iconv.decode(
          responseBuffer.slice(9, responseBuffer.length),
          "utf-16be"
        );
  ...
  static ping13(host: string, port = 25565, timeout = 3000) {
    return new Promise((resolve, reject) => {
      ...
      client.on("data", (responseBuffer) => {
        const dataBuffer = iconv.decode(
          responseBuffer.slice(3, responseBuffer.length),
          "utf-16be"
        );
