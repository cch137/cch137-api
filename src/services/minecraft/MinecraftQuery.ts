 ...
 *
 */
export default class MinecraftQuery {
  /*
   *	Basic stat
   *	https://wiki.vg/Query#Basic_stat
   *
   */
  static query(host: string, port = 25565, callback: Function, timeout = 3000) {
    ...
    return new Promise((resolve, reject) => {
      ...
      client.on("message", (responseBuffer) => {
        if (authenticated) {
          ...
          // Response
          const responseData = responseBuffer
            .toString("utf-8", 16)
            .split("\x00\x01player_\x00\x00");
