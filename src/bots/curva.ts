...
import { ApplicationCommandOptionType, IntentsBitField, Interaction, Message, MessageType, codeBlock } from 'discord.js';
import Mexp from 'math-expression-evaluator'
import { BotClient, createErrorEmbed, createInfoEmbed, createSuccessEmbed, createWarningEmbed, startTyping, toCodeBlocks } from './utils';
...
export const run = () => curva.connect()
  .then(async () => {
    ...
    newCommands
      .then(() => console.log(`${user.displayName} commands is ready.`))

    curva.on("messageCreate", async (message: Message) => {
      if (message.author.bot) return;
      const {channel} = message;
      if (message.type === MessageType.Reply) return;
      const isTriggeredByListening = listeningChannelIds.has(channel.id) && !answeringChannelIds.has(channel.id);
      ...
      try {
        ...
        const res = await askCh4Ai(
          messages.sort((a, b) => a.createdAt.getTime() - b.createdAt.getTime()).map((m) => {
        ...
        chunks.$on(async () => {
          ...
          try {
            ...
            if (r === l) return;
            r = l;
            let content = chunks.slice(i).join('')
              .replace(/@everyone/g, 'everyone')
              .replace(/@here/g, 'here')
              .replace(/^<@[0-9]*>( :)?(:)?/, '');
            if (content.length > 2000) {
              i = j;
              content = chunks.slice(i).join('')
                .replace(/@everyone/g, 'everyone')
                .replace(/@here/g, 'here');
              replied = (await replied).channel.send({content})
                .then((m) => (j = l, m)) as Promise<Message<false>> | Promise<Message<true>>;
            } else {
              replied = (await replied).edit({content, embeds: []})
                .then((m) => (j = l, m)) as Promise<Message<false>> | Promise<Message<true>>;
            }
          } catch {}
        });
    ...
    curva.on('interactionCreate', async (interaction: Interaction) => {
      ...
      switch (interaction.commandName) {
        ...
        case 'set-selected-messages': {
          ...
          ctxt = typeof v === 'number'
            ? v || 1
            : 16;
          interaction.reply(createSuccessEmbed(`setted selected messages: ${ctxt}`));
          break;
        }
        case 'config': {
          ...
          interaction.reply(createInfoEmbed(`model: ${model}\nselected messages: ${ctxt}\n${listenMsg}`));
          break;
        }
        case 'listen': {
        ...
        case 'unlisten': {
          ...
          interaction.reply(createWarningEmbed('stopped listening to this channel.'));
          break;
        }
        case 'wikipedia': {
        ...
        case 'calc': {
          try {
            ...
            interaction.reply(createErrorEmbed('The expressions cannot be calculated.'));
          }
          break;
        }
      }
